...
class BotService {
    ...
    async initiateBotBidding(room, bots) {
        try {
            ...
            if (botCount >= 3) {
                console.log(`[BotService] Límite de pujas alcanzado en sala ${room}`);
                this.io.to(room).emit('auctionAlert', {
                    message: 'Solo se pueden hacer 3 pujas en esta subasta. Esperando validación...',
                    type: 'warning'
                });
                return;
            }
            ...
            const timeout = setTimeout(async () => {
                try {
                    ...
                    if (result.success) {
                        this.botBidCounts.set(room, botCount + 1);
                        if (this.activeBotRooms.has(room) && botCount < 2) {
                            await this.initiateBotBidding(room, bots);
                        }
                    }
                } finally {
